...
3. Type `cmd` and press Enter. This opens a Command Prompt window already pointed at your project folder. **Important: always use `cmd`, not `powershell`.** PowerShell has a script execution policy that blocks npm from running.
...
You can close this Command Prompt window now.
...
**To stop the databases later** (when you're done for the day): open Docker Desktop, find the containers, and click the stop button. Or in Command Prompt: `docker compose down`
...
**npm install fails with "node is not recognised":** Node.js didn't install correctly or the system PATH wasn't updated. Try closing and reopening the Command Prompt window. If that doesn't help, restart your computer. Also make sure you are using `cmd`, not PowerShell.
