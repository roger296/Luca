...
async function deliverEvent(eventType: WebhookEventType, data: object): Promise<void> {
  ...
  for (const sub of subscriptions as Array<Record<string, unknown>>) {
    ...
    const [deliveryRow] = await knex("webhook_deliveries")
      .insert({
...
export async function processRetryQueue(): Promise<void> {
  ...
  const retryRows = await knex("webhook_deliveries")
  ...
  for (const rawRow of retryRows as Array<Record<string, unknown>>) {
    ...
    if (lastAttemptAt !== null) {
      ...
      const nextAttemptAt = new Date(lastAttemptAt.getTime() + delayMs);
    ...
    if (delivered) {
      ...
      await knex("webhook_subscriptions").where("id", subscriptionId).update({
