...
export async function getDocumentsForTransaction(
  ...
  return knex("transaction_documents")
...
export async function getDocumentById(id: string): Promise<TransactionDocument | undefined> {
  return knex("transaction_documents").where({ id }).first();
}
...
export async function insertDocument(doc: {
  ...
  const [inserted] = await knex("transaction_documents").insert(doc).returning("*");
  return inserted;
}

export async function deleteDocument(id: string): Promise<void> {
  await knex("transaction_documents").where({ id }).delete();
}
