...
The platform is defined by two foundational principles: **modularity** and **AI agent access as a first-class interface**.

Modularity ensures that each functional area is delivered as a separate module that communicates with other modules exclusively through published APIs. This architecture enables the platform vendor to develop and release modules independently, third-party developers and businesses to build their own modules that integrate seamlessly with the platform, and individual businesses to adopt only the modules they need while retaining the ability to expand over time.

AI agent access is equally fundamental. Every module must publish an MCP (Model Context Protocol) server as a mandatory interface. This reflects a core belief: AI agents like Claude operating in Cowork mode are expected to be a primary day-to-day interface for many businesses, not a future capability or an afterthought. The platform provides three equal access methods — web UI, REST API, and MCP server — each a first-class citizen in the architecture. Businesses choose their interface based on how they work, and the platform natively supports all three with identical authorization, audit, and workflow controls.

The APIs (including the MCP server) are the product as much as the modules themselves. By publishing stable, well-documented APIs across all three channels, the platform becomes an extensible ecosystem that humans and AI agents can both navigate seamlessly.
...
Modules in the ecosystem can be understood in three tiers based on their relationship with the GL. Regardless of tier, every module must expose three interfaces: a **REST API** for programmatic access, a **web frontend** for human users, and an **MCP server** for AI agents. The MCP server is mandatory, not optional — it is a core architectural requirement that ensures AI agents have native access to all platform capabilities on equal footing with human users and traditional API clients.
...
- A consistent header showing the company name, the current accounting period and its status, and the logged-in user's role.
...
### 11.1 Purpose and Architectural Status
...
This is achieved by building an **MCP (Model Context Protocol) server** as a fundamental architectural component, equal in importance to the REST API and web UI. The three access methods form a triad: **web UI for human interaction**, **REST API for system-to-system integration**, and **MCP server for AI agent autonomy**. Each is a first-class citizen with identical authorization, audit, and workflow controls.
...
Importantly, AI agents are expected to be the primary day-to-day interface for many small to medium businesses. A business owner or accountant may predominantly interact with the platform through an AI assistant running in Cowork mode, using the web UI only for exception handling and review. This is not a future scenario — it is a design expectation that shapes every architectural decision. The platform must be as native to AI as it is to humans.

The MCP server sits alongside the REST API and web frontend as an equal interface to the same underlying services. All three call the same backend logic and are subject to the same authentication, authorisation, and audit trail requirements.
...
- **Authentication** via OAuth 2.0 tokens, scoped to a specific user identity. The agent acts on behalf of a named user, and all transactions it submits are recorded as originating from that user via the MCP interface.
...
### 12.3 Single-Tenant Architecture (One Instance Per Company)

The platform is designed for single-tenant deployment — each instance runs for one company only. Multi-tenant architecture is not supported. Data isolation, security, and regulatory compliance are ensured by deploying separate instances rather than sharing infrastructure. Each company's chain files and database records are completely separate by design, making this architecture inherently secure and audit-friendly.
...
- **AI agent access as a first-class interface** — the platform is architected from the ground up with AI agents as expected primary users, not as an afterthought. Every module publishes an MCP server alongside its REST API and web UI. This is not about adding AI features; it is about making the entire platform native to agent operation. Businesses operating through AI assistants in Cowork mode have the same first-class experience as those using the web interface. No incumbent offers this architectural parity.

- **Three equal access methods** — web UI for human users, REST API for system integrations, and MCP server for AI agents. All three receive identical treatment in terms of authorization, audit trails, approval workflows, and feature parity. This eliminates the second-class citizen experience common in platforms that bolt on API access or AI capabilities after the fact.
